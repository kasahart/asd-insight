...
マニュアルには、実画面を使った約2分の操作動画、日本語字幕、練習用のCSV/WAV（`inspection.csv` / `tone.wav`）を含みます。字幕はGitHub PagesなどHTTPで配信されたマニュアルで確認してください。`file://` で直接開くと、ブラウザーの制限で字幕を読み込めない場合があります。
